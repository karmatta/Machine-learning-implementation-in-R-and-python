...
flog.appender(appender.file(paste(mripPath,"/MINTlogs/corAgentGB.log",sep="")), name=logNS)
...
createJson <- function(mat){
  
  jsonMst ="\"graph\":["
  
  values<- mat[lower.tri(mat, diag = F)]
  dims <- which(lower.tri(mat, diag = F), arr.ind =T)-1
...
# Main function
#==============
MainComputeCor <- function(inputDfJson){
  ...
  flog.info("%s Json sent succesfully.",whichMat,name=logNS)
  prevJson <<- retJson
  retJson <- paste(retJson,",",fieldsJson,"}",sep="")
  return(retJson)
}
